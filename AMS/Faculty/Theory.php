...
<div class="container-fluid">
      <div class="row">
		...
		<br>
<h3 align="center">Theory Attendance</h3>
<hr>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="user_form" method="post" action="ajax.php">
							...
							?>
								...
								<div class="form-group">
									...
									<?php
									//$week=date("D"); 
									/*$get_time="select StartTime,EndTime from timetable where mon like '$sub'";
									$result_time=mysqli_query($connection, $get_time);
									$row_time = mysqli_fetch_assoc($result_time);
									$StartTime=$row_time['StartTime'];
									$EndTime=$row_time['EndTime'];
									$minus=$EndTime-$StartTime;*/
									//echo $week;
									
									//$get_time="select Time from timetable where ";
									?>
									<input type="text" placeholder="Duration" value="1.5" name="Duration" class="form-control" required>
								</div>
								<div class="form-group">
									<label>Date</label>									
...
<?php
	...
	$a=0;$i=0;
	$sql="select e.stud_id,s.stud_name from enroll e,student s where e.course_id='$course_id' and e.sub_id='$sub_id' and s.course_id = '$course_id' and s.stud_id=e.stud_id";
	...
	echo "<form id='thisForm' name='myform' method='POST' action='ajax.php'>";
