...
$stud="select * from facultylogin where username='$username'";
...
$get_email="select * from faculty where fact_id='$fact_id' and dept_name='$dept_name'";
...
$sql = "UPDATE facultylogin SET password='$tempPass' WHERE username='$username'";
...
echo "<b>Dear $row_email[faculty_name] $row_email[Last_Name]</b><br>";
...
$get_email="select * from faculty where email_id='$email'";
$query_email=mysqli_query($con,$get_email);

if(mysqli_num_rows($query_email)>0){
	$row_email = mysqli_fetch_assoc($query_email);
	
	$fact_id=$row_email['fact_id'];
	$dept_name=$row_email['dept_name'];
	
	$get_user="select * from facultylogin where fact_id='$fact_id' and dept_name='$dept_name'";
	...
	$randNum = rand(10000,99999); //generates a random code
	$tempPass = "$randNum";
	$sql2 = "UPDATE facultylogin SET password='$tempPass' WHERE username='$username'";
