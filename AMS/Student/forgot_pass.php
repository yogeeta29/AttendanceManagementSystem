...
<body style="background-color:#e6e6ff;font-family:Verdana;font-size:16px;">
<header>
...
$stud="select stud_id,course_id from logindetails where username='$username'";
...
$get_email="select email_id,stud_name from student where stud_id='$stud_id' and course_id='$course_id'";
$query_email=mysqli_query($con,$get_email);
$row_email = mysqli_fetch_assoc($query_email);
$email_id=$row_email['email_id'];
$stud_name=$row_email['stud_name'];

$randNum = rand(10000,99999); //generates a random code
$tempPass = "$randNum";
//$hashTempPass = md5($tempPass);
$sql = "UPDATE logindetails SET password='$tempPass' WHERE username='$username'";
...
$get_email="select * from student where email_id='$email'";
$query_email=mysqli_query($con,$get_email);

if(mysqli_num_rows($query_email)>0){
	$row_email = mysqli_fetch_assoc($query_email);
	$stud_name=$row_email['stud_name'];
	$stud_id=$row_email['stud_id'];
	$course_id=$row_email['course_id'];
	
	$get_user="select * from logindetails where stud_id='$stud_id' and course_id='$course_id'";
	$query_user=mysqli_query($con,$get_user);
	$row_user = mysqli_fetch_assoc($query_user);
	$username=$row_user['username'];	
	
	$randNum = rand(10000,99999); //generates a random code
	$tempPass = "$randNum";
	$sql2 = "UPDATE logindetails SET password='$tempPass' WHERE username='$username'";
